...
export class NextStandaloneStack extends Stack {
	...
	constructor(scope: App, id: string, config: CustomStackProps) {
		...
		// 	})
		// }
		if (config.domainNames?.length && config.acmCertificateArn) this.cfnCertificate = this.getCfnCertificate(config.acmCertificateArn)
		...
		this.cfnDistro = this.setupCfnDistro({
			...
			description: config.cloudfrontDescription,
		})
	...
	setupCfnDistro({ apiGateway, imageBasePath, serverBasePath, assetsBucket, domainNames, certificate, description }: SetupCfnDistroProps) {
	...
	getCfnCertificate(acmCertificateArn: string) {
		...
		const certificate = Certificate.fromCertificateArn(this, 'Certificate', acmCertificateArn)

		new CfnOutput(this, 'certificateArn', { value: acmCertificateArn })
